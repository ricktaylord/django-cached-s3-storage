...
class CachedS3BotoStorage(S3BotoStorage):
	...
	def entries(self):
		...
		"""
		return dict([(k, v) for k,v in self.raw_entries().iteritems() if v.size>0])

	def raw_entries(self):
		if not self._entries:
			self._entries = dict([(self._clean_name(entry.path), entry) for entry in S3FileMeta.objects.all()])
	...
	def folders(self):
		...
		re-calculated from the entries() cache each time, as last-modified not available directly from S3
		"""
	...
	def _update_db_cache_entry(self, name, fle=None, thumbnail=False, s3key=None, update=True):
		if s3key is None:
			try:
				s3key = fle.key
			except:
				s3key = self.bucket.get_key(self._normalize_name(self._clean_name(name)))
		filemeta_defaults = {'size': s3key.size, 'last_modified':parse_ts_extended(s3key.last_modified)}
		if name not in self.entries or self.entries[name].image_x is None:
			...
			try:
				...
				im = Image.open(fle)
				filemeta_defaults.update({'image_x':im.size[0],'image_y':im.size[1]})
		...
		if name not in self.entries or update or self.entries[name].image_x is None:
			filemeta, created = S3FileMeta.objects.get_or_create(path=name, defaults=filemeta_defaults)
			if not created:
	...
	def listmostrecent(self):
		return ([],[el.path for el in S3FileMeta.objects.filter(tags=None).order_by("-last_modified")[0:100]])
	...
	def listdir(self, name):
		...
		level = name.count("/")
		lpath = len(name)+1
		files = [ path[lpath:] for path in self.entries.keys() if path.startswith(name) and path not in self.folders.keys() and path.count("/") == level+1 ]
		folders = [ path[lpath:] for path in self.folders.keys() if path.startswith(name) and  path.count("/") == level+1 ]
	...
	def listdir_bytag(self,name):
		...
		paths = [el.path for el in S3FileMeta.objects.filter(tags__name=last_seg)]
		if not paths:
			if last_seg==RECENT_UPLOAD_DIRECTORY:
				return self.listmostrecent()
			return ([el.name for el in FileTag.objects.all()]+[self.recent_upload_directory,],[])
		return (["/",],paths)
	...
	def listdir_recursive(self, name, raw=False, full=False):
		...
		files = [ outfn(path) for path in entr if path.startswith(name) and path not in self.folders.keys()]
		folders = [ outfn(path) for path in self.folders.keys() if path.startswith(name)]
		return (folders,files)
	...
	def sync_caches_to_S3(self):
		...
		for key in blist:
			print key
			self._update_db_cache_entry(key.name[len(self.location):], fle=None, s3key=key, update=False)
	...
	def convert_all_images_to_RGB(self):
		for path,fl in self.entries.iteritems():
			try:
				...
				im.convert('RGB').save(tmpfile, format=Image.EXTENSION[ext.lower()], quality=95)
	...
	def isdir(self, path):
		...
		if USE_TAG_DIRECTORIES:
			...
			logging.debug("Checking if "+path+" is a folder or "+last_seg+" is a tag")
			if path in self.entries.keys() and path not in self.folders.keys():
				return False
			return self._clean_name(path) in self.folders.keys() or FileTag.objects.filter(name=self._clean_name(last_seg)).exists() or last_seg=="recent_uploads"
	...
	def _save(self, path, fle,*args,**kwargs):
		...
		fle.seek(0) #ensures we are at the start of the file and makes filebrowser thumbnails work.
		saveret = super(CachedS3BotoStorage,self)._save(path,fle,*args,**kwargs)
		try:
	...
	def mvtree(self, old_dir, new_dir):
		if self.exists(new_dir):
			raise OSError("The folder '%s' already exists: please choose a different name" % new_dir)
	...
	def mvfile(self, old_file_name, new_file_name, allow_overwrite=False):
		if self.exists(new_file_name):
			if allow_overwrite:
				...
				raise OSError("The destination file '%s' exists and allow_overwrite is False" % new_file_name)
		...
		if not k:
			raise OSError("Couldn't copy '%s' to '%s'" % (old_key_name, new_key_name))
